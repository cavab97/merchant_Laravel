...
@section('multiple-action-button')
    @if($currentWorkspace && $currentWorkspace->permission == 'Owner')
        <a href="#" class="btn btn-xs btn-white btn-icon-only width-auto" data-ajax-popup="true" data-size="lg"
           data-title="{{ __('Create New Task') }}" data-url="{{route('tasks.create',[$currentWorkspace->slug,$project->id])}}"><i class="fas fa-plus mr-1"></i>{{ __('Add New')}}</a>
    @elseif(isset($permissions) && in_array('create task',$permissions))
        <a href="#" class="btn btn-xs btn-white btn-icon-only width-auto" data-ajax-popup="true" data-size="lg"
           data-title="{{ __('Create New Task') }}" data-url="{{route('client.tasks.create',[$currentWorkspace->slug,$project->id])}}"><i class="fas fa-plus mr-1"></i>{{ __('Add New')}}</a>
    @endif
                                                                    ...
                                                                    @if($currentWorkspace->permission == 'Owner' || isset($permissions))
                                                                        <div class="dropdown float-right">
                                                                            ...
                                                                            <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(22px, 31px, 0px);">
                                                                                <a href="#" class="dropdown-item" data-ajax-popup="true" data-size="lg" data-title="{{ __('View Task') }}" data-url="@auth('web'){{route('tasks.show',[$currentWorkspace->slug,$task->project_id,$task->id])}}@elseauth{{route('client.tasks.show',[$currentWorkspace->slug,$task->project_id,$task->id])}}@endauth">
                                                                                {{__('View')}}</a>
                                                                                ...
                                                                                @elseif(isset($permissions))
                                                                                    @if(in_array('edit task',$permissions))
                                                                                        <a href="#" class="dropdown-item" data-ajax-popup="true" data-size="lg" data-title="{{ __('Edit Task') }}" data-url="{{route('client.tasks.edit',[$currentWorkspace->slug,$task->project_id,$task->id])}}">
                                                                                            {{__('Edit')}}
                                                                                        </a>
                                                                                    @endif
                                                                                    @if(in_array('delete task',$permissions))
                                                                                        ...
                                                                                        <form id="delete-form-{{$task->id}}" action="{{ route('client.tasks.destroy',[$currentWorkspace->slug,$task->project_id,$task->id]) }}" method="POST" style="display: none;">
                                                                                            @csrf
                                                                                            @method('DELETE')
                                                                                        </form>
                                                                                    @endif
    ...
    @push('scripts')
        ...
        <script>
            !function (a) {
                ...
                t.prototype.init = function () {
                    a('[data-toggle="dragula"]').each(function () {
                        ...
                        r ? dragula(n, {
                            ...
                            $.ajax({
                                url: '@auth('web'){{route('tasks.update.order',[$currentWorkspace->slug,$project->id])}}@elseauth{{route('client.tasks.update.order',[$currentWorkspace->slug,$project->id])}}@endauth',
                                type: 'PUT',
                                data: {
                                    id: id,
                                    sort: sort,
                ...
                "use strict";
                @auth('client')
                    @if(isset($permissions) && in_array('move task',$permissions))
                        a.Dragula.init()
                    @endif
                @elseauth
                    a.Dragula.init()

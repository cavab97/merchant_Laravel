...
@section('multiple-action-button')
    @if($currentWorkspace && $currentWorkspace->permission == 'Owner')
        <a href="#" class="btn btn-xs btn-white btn-icon-only width-auto" data-ajax-popup="true" data-size="lg"
           data-title="{{ __('Create New Bug') }}" data-url="{{route('projects.bug.report.create',[$currentWorkspace->slug,$project->id])}}"><i class="fas fa-plus mr-1"></i>{{ __('Add New')}}</a>
    @elseif(isset($permissions) && in_array('bug report',$permissions))
        <a href="#" class="btn btn-xs btn-white btn-icon-only width-auto" data-ajax-popup="true" data-size="lg"
           data-title="{{ __('Create New Bug') }}" data-url="{{route('client.projects.bug.report.create',[$currentWorkspace->slug,$project->id])}}"><i class="fas fa-plus mr-1"></i>{{ __('Add New')}}</a>
    @endif
    @auth('client')
                                            ...
                                            @foreach($stage->bugs as $bug)
                                                <div class="card card-progress draggable-item border shadow-none mb-3" id="{{$bug->id}}">
                                                    <div class="card-body">
                                                                    ...
                                                                    @if($currentWorkspace->permission == 'Owner' || isset($permissions))
                                                                        <div class="dropdown float-right">
                                                                            ...
                                                                            <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(22px, 31px, 0px);">
                                                                                    <a href="#" class="dropdown-item" data-ajax-popup="true" data-size="lg" data-title="{{ __('View Bug') }}" data-url="@auth('web'){{route('projects.bug.report.show',[$currentWorkspace->slug,$bug->project_id,$bug->id])}}@elseauth{{route('client.projects.bug.report.show',[$currentWorkspace->slug,$bug->project_id,$bug->id])}}@endauth">
                                                                                        {{__('View')}}</a>
                                                                                ...
                                                                                @elseif(isset($permissions))
                                                                                    @if(in_array('edit bug report',$permissions))
                                                                                        <a href="#" class="dropdown-item" data-ajax-popup="true" data-size="lg" data-title="{{ __('Edit Bug') }}" data-url="{{route('client.projects.bug.report.edit',[$currentWorkspace->slug,$bug->project_id,$bug->id])}}">
                                                                                            {{__('Edit')}}
                                                                                        </a>
                                                                                    @endif
                                                                                    @if(in_array('delete bug report',$permissions))
                                                                                        ...
                                                                                        <form id="delete-form-{{$bug->id}}" action="{{ route('client.projects.bug.report.destroy',[$currentWorkspace->slug,$bug->project_id,$bug->id]) }}" method="POST" style="display: none;">
                                                                                            @csrf
                                                                                            @method('DELETE')
                                                                                        </form>
                                                                                    @endif
                                                        ...
                                                        <div class="row">
                                                            <div class="col-12">
                                                                <a href="#" data-url="@auth('web'){{route('projects.bug.report.show',[$currentWorkspace->slug,$bug->project_id,$bug->id])}}@elseauth{{route('client.projects.bug.report.show',[$currentWorkspace->slug,$bug->project_id,$bug->id])}}@endauth" data-ajax-popup="true" data-title="{{__('Bug Detail')}}" class="h6 task-title">{{$bug->title}}</a>
                                                            </div>
                                                        </div>
                                                        <div class="row">
                                                            <div class="col">
    ...
    @push('scripts')
        ...
        <script>
            !function (a) {
                ...
                t.prototype.init = function () {
                    a('[data-toggle="dragula"]').each(function () {
                        ...
                        r ? dragula(n, {
                            ...
                            $.ajax({
                                url:'@auth('web'){{route('projects.bug.report.update.order',[$currentWorkspace->slug,$project->id])}}@elseauth{{route('client.projects.bug.report.update.order',[$currentWorkspace->slug,$project->id])}}@endauth',                                type: 'PUT',
                                data: {
                                    id: id,
                                    sort: sort,
                ...
                "use strict";
                @if(isset($permissions) && in_array('move task',$permissions) || $currentWorkspace->permission == 'Owner')
                    a.Dragula.init()
                @endif
            }(window.jQuery);
        </script>

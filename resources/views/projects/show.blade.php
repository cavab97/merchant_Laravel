                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            ...
                                            <div class="text-right">
                                                @if((isset($permissions) && in_array('create milestone',$permissions)) || $currentWorkspace->permission == 'Owner')
                                                    <a href="#" class="btn btn-xs btn-info" data-ajax-popup="true" data-title="{{ __('Create Milestone') }}" data-url="@auth('web'){{route('projects.milestone',[$currentWorkspace->slug,$project->id])}}@elseauth{{route('client.projects.milestone',[$currentWorkspace->slug,$project->id])}}@endauth"><i class="fas fa-plus"></i></a>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                    <div class="list-group list-group-flush">
                                        @foreach($project->milestones as $key => $milestone)
                                            <div class="list-group-item">
                                                <div class="row align-items-center">
                                                    <div class="col">
                                                        <a href="#" class="d-block font-weight-500 mb-0" data-ajax-popup="true" data-title="{{ __('Milestone Details') }}" data-url="@auth('web'){{route('projects.milestone.show',[$currentWorkspace->slug,$milestone->id])}}@elseauth{{route('client.projects.milestone.show',[$currentWorkspace->slug,$milestone->id])}}@endauth">
                                                            {{$milestone->title}}
                                                        </a>
                                                        <small>
                                                        ...
                                                        @auth('web')
                                                            ...
                                                            @elseif(isset($permissions))
                                                                @if(in_array('edit milestone',$permissions))
                                                                    <a href="#" class="edit-icon" data-ajax-popup="true" data-size="lg" data-title="{{__('Edit Milestone')}}" data-url="{{route('client.projects.milestone.edit',[$currentWorkspace->slug,$milestone->id])}}"><i class="fas fa-pencil-alt"></i></a>
                                                                @endif
                                                                @if(in_array('delete milestone',$permissions))
                                                                    <a href="#" class="delete-icon" data-confirm="{{ __('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?') }}" data-confirm-yes="document.getElementById('delete-form1-{{$milestone->id}}').submit();"><i class="fas fa-trash"></i></a>
                                                                    <form id="delete-form1-{{$milestone->id}}" action="{{ route('client.projects.milestone.destroy',[$currentWorkspace->slug,$milestone->id]) }}" method="POST" style="display: none;">
                                                                        @csrf
                                                                        @method('DELETE')
                                                                    </form>
                                                                @endif
...
@push('scripts')
    ...
    <script src="{{asset('assets/js/dropzone.min.js')}}"></script>
        ...
        dropzoneBtn(mockFile, {download: "@auth('web'){{route('projects.file.download',[$currentWorkspace->slug,$project->id,$file->id])}}@elseauth{{route('client.projects.file.download',[$currentWorkspace->slug,$project->id,$file->id])}}@endauth", delete: "{{route('projects.file.delete',[$currentWorkspace->slug,$project->id,$file->id])}}"});
